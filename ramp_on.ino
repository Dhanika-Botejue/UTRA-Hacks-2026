...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println("Reading Analog IR signal...");
...
if (IR == true) {
  ...
  if (irSensor1RawValue < 500 && irSensor2RawValue < 500) {
      analogWrite(ENA, 225);
      analogWrite(ENB, 225);
      moveForward();
      delay(100);
  ...
  // 1. THE MAIN FORWARD LOOP
  // While color is NOT white, keep moving forward
